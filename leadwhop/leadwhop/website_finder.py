...
def normalize_url(url: str) -> str:
    """Strip tracking params and deep paths -> clean homepage URL.

    'https://x.com/policies/terms?srsltid=abc' -> 'https://x.com/'
    """
...
class WebsiteFinder:
    ...
    def _verify_by_content(self, company: str, website: str) -> str:
        ...
        verdict = self.llm.json_call(
            system=("You verify whether a homepage belongs to a specific "
                    ...
                    "industry is enough. HOWEVER: directories, registries, "
                    "award sites, or pages listing MANY companies are NOT "
                    "the company's own site — answer No for those. "
    ...
    def find(self, company: str, country: str = "") -> dict:
        ...
        # Second-chance search: plain company name, no quotes/"official
        # website". Brand-named sites (Wilkin & Sons -> tiptree.com) often
        # only surface in a plain query. Costs +1 Serper credit, but only
        # for the failing minority.
        if not candidates:
            try:
                results2 = self._search(f"{company} {country}".strip())
                candidates = [
                    {"link": r.get("link", ""), "title": r.get("title", ""),
                     "snippet": r.get("snippet", "")}
                    for r in results2 if self._is_candidate(r.get("link", ""))
                ]
            except requests.RequestException:
                pass
        ...
        if not candidates:
            return {"website": "", "domain": "", "confidence": "none_found",
                    "needs_review": True,
                    "debug": f"No usable candidates after 2 searches "
                             f"(all blocked by blacklist or empty)"}
        ...
        for cand in candidates:
            ...
            if self._name_token_in_domain(company, domain):
                return {"website": normalize_url(cand["link"]),
                        "domain": domain,
        ...
        verdict = self.llm.json_call(
            system=(
                ...
                "IMPORTANT: company registries, award/certification "
                "directories, chambers of commerce, and listing sites that "
                "feature MANY companies are NEVER the official website. "
        ...
        # STRUCTURAL RULE: a domain that fails the name heuristic is NEVER
        # accepted on GPT confidence alone — content verification is
        # mandatory. This is what catches directories, registries and
        # brand-domain mismatches (Wilkin & Sons -> tiptree.com) alike.
        outcome = self._verify_by_content(company, website)
        if outcome == "verified":
            return {"website": website, "domain": domain,
                    "confidence": "verified", "needs_review": False,
                    "debug": f"content-verified '{domain}' "
                             f"(name-domain mismatch resolved by homepage)"}
            ...
            return {"website": website, "domain": domain,
                    "confidence": "unverified", "needs_review": True,
                    "debug": f"site unreachable for content check, "
                             f"keeping GPT pick '{domain}' — review manually"}
