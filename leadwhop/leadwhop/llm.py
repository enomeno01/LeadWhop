...
class LLM:
    ...
    def _cache(self, namespace: str) -> DiskCache:
        # Namespaced per model: switching models must never reuse verdicts or
        # classifications produced by a different one.
        safe_model = re.sub(r"[^A-Za-z0-9._-]", "_", self.model)
        key = f"{namespace}__{safe_model}"
        if key not in self._caches:
            self._caches[key] = DiskCache(self._cache_dir, key)
    ...
    def choose(self, namespace: str, system: str, user: str,
        ...
        try:
            ...
            if "insufficient_quota" in str(exc) or "429" in str(exc):
                status.warn("OpenAI: OUT OF CREDITS or rate limited — check platform.openai.com billing.")
            return default
    ...
    def json_call(self, system: str, user: str, max_tokens: int = 120) -> dict:
        ...
        try:
            ...
            if "insufficient_quota" in msg or "429" in msg:
                ...
                status.warn(
                    f"OpenAI: model '{self.model}' is not available on this API key. "
                    f"Change models.llm in config/settings.yaml (e.g. gpt-4o-mini)."
                )
            return {}
