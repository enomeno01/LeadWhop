...
FIRST CHECK — answer this BEFORE anything else:
Is this company a PACKAGING MANUFACTURER? (its PRODUCT is packaging itself:
glass bottles/jars, plastic containers, metal cans, caps, closures, labels,
cartons). If YES -> GlassFit = "No" IMMEDIATELY and stop. These companies
SELL packaging — they are competitors or suppliers, never glass BUYERS.
Example: a glass bottle producer's products ARE glass, but GlassFit = No.

Otherwise: does this company's product line typically use glass packaging?
...
2. (Packaging manufacturers already handled by FIRST CHECK above.)
...
DECISION RULE — DEFAULT IS "Manufacturer":
- Wineries, distilleries, breweries, estates, mills, farms, food processors
  = Manufacturer, ALWAYS (they produce by definition).
- Classify "Co-packer" ONLY if snippets explicitly mention contract filling,
  co-packing, or private-label production FOR OTHER BRANDS.
- Classify "Brand Owner" ONLY if snippets explicitly mention outsourced
  production ("produced by partners", "made for us by").
- Classify "Distributor" ONLY if snippets show importing/distributing with
  NO own-brand production.
- Absence of facility information is NOT evidence of outsourcing.
  No explicit evidence = "Manufacturer".
