    ...
    def run(self, input_path: str, stages: list[str],
        ...
        if "export" in stages and self.exporter:
            ...
            if has_contacts:
                ...
                self.crm_df.to_csv(self.output_dir / "crm_import.csv",
        ...
        if "mail" in stages:
            df = self.drafter.run(df)
            mail_cols = [c for c in ("Name", "Company", "Email",
                                     "Email_Subject", "Email_Draft")
                         if c in df.columns]
            mail_df = df[df.get("Email_Draft", "") != ""][mail_cols]
            if len(mail_df):
                mail_df.to_excel(self.output_dir / "mail_drafts.xlsx",
                                 index=False)
