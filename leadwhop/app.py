...
with st.sidebar:
    ...
    with st.expander("⚙️ Advanced settings"):
        ...
        if "qualify" in selected:
            st.markdown("**Product fit — extra instructions**")
            st.caption("Add anything you want the AI to consider when deciding "
                       "if a company fits your ICP. Leave blank to use defaults.")
            custom_icp_prompt = st.text_area(
                "ICP instructions",
                placeholder="e.g. We also sell special spirits bottles with "
                            "engraving — include craft distilleries even if small.",
                height=100, label_visibility="collapsed",
            )
        if "mail" in selected:
            st.markdown("**Email — extra instructions**")
            st.caption("Tell the AI how to write the personalised hook. "
                       "Leave blank to use defaults.")
            custom_mail_prompt = st.text_area(
                "Mail instructions",
                placeholder="e.g. Mention our new matte black bottle finish. "
                            "Keep tone formal. Avoid mentioning price.",
                height=100, label_visibility="collapsed",
            )
...
if uploaded:
    ...
    if missing_keys:
        ...
        if run:
            ...
            if crm_df is not None and len(crm_df):
                with tabs[tab_i]:
                    st.dataframe(crm_df, use_container_width=True, height=350)
                    st.download_button(
            ...
            if mail_df is not None and len(mail_df):
                with tabs[tab_i]:
                    st.dataframe(mail_df, use_container_width=True, height=350)
