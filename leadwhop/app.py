...
with st.sidebar:
    ...
    # Dependency guard: enforce logical order
    if "export" in selected and "contacts" not in selected:
        st.warning("⚠️ Stage 5 requires Stage 3 (Find buyers). Adding it automatically.")
        if "contacts" not in selected:
            selected.insert(selected.index("export"), "contacts")
    if "phones" in selected and "contacts" not in selected:
        selected.insert(selected.index("phones"), "contacts")
    if "contacts" in selected and "qualify" not in selected:
        st.info("ℹ️ Tip: Stage 2 (Check product fit) filters companies before spending Lusha credits.")
